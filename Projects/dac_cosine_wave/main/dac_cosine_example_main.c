...
//create an enum here of stop and start and pause
enum Data_List_t{
    Stop = 1 << 0,
    Start = 1 << 1,
    Pause = 1 << 2,
...
static void BitSenderTask(void *args)
{
    //This task send the bits
    //Send Start
    //Wait 10 secs
    //Send Stop
    xEventGroupSetBits(eventGroup, START);
    vTaskDelay(pdMS_TO_TICKS(10000));  
    xEventGroupSendBits(eventGroup, STOP); 
    for (;;)
    {
        vTaskDelay(pdMS_TO_TICKS(10000));
    }
...
static void TimerControllerTask(void *args)
{
    for(;;)
    {
        //Wait for every event group
        ...
        //Check coresponding even group and do stuff with timer.
        switch (uxBits) {
        case Start | Stop:
            printf ( " Both eBit0 and eBit1 were set .\ n " ) ;
            break;
        case Stop:
            printf("PAUSE bit set. Pausing the timer.\n");
            xTimerStop(xMyTimer, 0);
            break;
        case Start:
            printf("START bit set. Starting the timer.\n");
...
void vTimerCallback(TimerHandle_t xTimer)
{
    ...
    if(xMyQueue == NULL){
        ...
        xEventGroupSetBits(eventGroup, STOP);
        return;
    }
    else
    {
        xQueueSend(xMyQueue, &counter,0);
    }
}

void app_main(void) 
{
    //Create a periodic timer with 5 
    //seconds period and each callback increments a value that is send to a queue.
    //If start bit is set we start the timer if stop bit is set we stop timer
    // We have other task to stop or start sending bits.

    xMyTimer = xTimerCreate("Timer1", 5000/portTICK_PERIOD_MS, pdTRUE, 0, vTimerCallback)
    xMyQueue = xQueueCreate(10, sizeof(unsigned long));
    eventGroup = xEventGroupCreate();
    xTaskCreate(TimerControllerTask, "TimerController Task", 4096, NULL, 5, NULL);
}
